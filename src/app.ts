...
const chart = mustElement<SVGSVGElement>("chart");
const datasetSelect = mustElement<HTMLSelectElement>("datasetSelect");
const csvInput = mustElement<HTMLTextAreaElement>("csvInput");
const csvStatus = mustElement<HTMLElement>("csvStatus");
const loadDummyButton = mustElement<HTMLButtonElement>("loadDummyButton");
const loadCsvButton = mustElement<HTMLButtonElement>("loadCsvButton");
const autoBalanceButton = mustElement<HTMLButtonElement>("autoBalanceButton");
const downloadSvgButton = mustElement<HTMLButtonElement>("downloadSvgButton");
const downloadCsvButton = mustElement<HTMLButtonElement>("downloadCsvButton");
const downloadJsonButton = mustElement<HTMLButtonElement>("downloadJsonButton");
const downloadReportButton = mustElement<HTMLButtonElement>("downloadReportButton");
const additionTime = mustElement<HTMLInputElement>("additionTime");
...
const readouts = {
  ...
  rightArea: mustElement<HTMLElement>("rightAreaReadout"),
  areaDiff: mustElement<HTMLElement>("areaDiffReadout"),
  fitAreaDiff: mustElement<HTMLElement>("fitAreaDiffReadout"),
  preFit: mustElement<HTMLElement>("preFitReadout"),
  postFit: mustElement<HTMLElement>("postFitReadout"),
  initialTemp: mustElement<HTMLElement>("initialTempReadout"),
  finalTemp: mustElement<HTMLElement>("finalTempReadout"),
  balanceStatus: mustElement<HTMLElement>("balanceStatus"),
};

const datasets = createDatasets();
let currentDataset = datasets[0]!;
let points = currentDataset.points;
let dragMode: "addition" | "corrected" | null = null;
...
function configureRanges(dataset: Dataset) {
...
function normalizeRanges(changed: HTMLInputElement) {
  ...
  if (changed === preEnd) pre = Math.min(pre, add - gap);
  if (changed === postStart) post = Math.max(post, add + gap);
...
function renderChart(preFit: Fit, postFit: Fit, _areas: AreaResult) {
  const scale = makeScale(points);
  const minTime = minT(points);
  const maxTime = maxT(points);
  ...
  addText(
    clamp(scale.x(corrected) + 5, scale.margin.left + 4, scale.width - scale.margin.right - 54),
    scale.margin.top + (scale.mobile ? 32 : 36),
    correctedLabel,
    "chart-label marker-label",
  );
...
function renderReadouts(preFit: Fit, postFit: Fit, initialTemp: number, finalTemp: number, delta: number, areas: AreaResult) {
  ...
  readouts.balanceStatus.textContent =
    Math.abs(areas.diff) <= tolerance ? "Areas casi iguales" : areas.diff > 0 ? "Mover t corregido a la derecha" : "Mover t corregido a la izquierda";
}
...
function createDatasets(): Dataset[] {
  return [
    ...
    },
    {
      ...
      postStart: 94,
      correctedTime: 70,
      points: syntheticCurve({ base: 20.0, drift: -0.0007, jump: 7.9, tauRise: 13, tauCool: 0.0042, tAdd: 58 }),
    },
  ];
}

function syntheticCurve(options: { base: number; drift: number; jump: number; tauRise: number; tauCool: number; tAdd: number }): Point[] {
  const result: Point[] = [];
...
function toCsv(data: Point[]) {
  return data.map((point) => `${point.t}, ${point.temp}`).join("\n");
}

function fitForRange(data: Point[], from: number, to: number): Fit {
...
function integrateDifference(data: Point[], preFit: Fit, postFit: Fit, from: number, to: number, side: "left" | "right") {
  if (to <= from) return 0;
...
function findBalancedTime(data: Point[], preFit: Fit, postFit: Fit, preLimit: number, postLimit: number) {
  let best = preLimit;
...
function sampleData(data: Point[], from: number, to: number): Point[] {
  const clipped = data.filter((point) => point.t > from && point.t < to);
  return [interpolate(data, from), ...clipped, interpolate(data, to)];
}

function interpolate(data: Point[], t: number): Point {
  const first = data[0]!;
  const last = data[data.length - 1]!;
  if (t <= first.t) return { t, temp: first.temp };
  if (t >= last.t) return { t, temp: last.temp };
...
function makeScale(data: Point[]): Scale {
...
function minY(data: Point[], preFit: Fit, postFit: Fit) {
  const values = data.flatMap((point) => [point.temp, evaluate(preFit, point.t), evaluate(postFit, point.t)]);
  return Math.min(...values) - 0.8;
}

function maxY(data: Point[], preFit: Fit, postFit: Fit) {
  const values = data.flatMap((point) => [point.temp, evaluate(preFit, point.t), evaluate(postFit, point.t)]);
  return Math.max(...values) + 0.8;
}

function linePath(data: Point[], scale: Scale) {
  return data.map((point, index) => `${index === 0 ? "M" : "L"} ${scale.x(point.t)} ${scale.y(point.temp)}`).join(" ");
}

function buildAreaPath(data: Point[], preFit: Fit, postFit: Fit, from: number, to: number, scale: Scale, side: "left" | "right") {
  if (to <= from) return "";
  const samples = sampleData(data, from, to);
...
function addGrid(scale: Scale, minTime: number, maxTime: number, minTemp: number, maxTemp: number) {
...
function addPath(d: string, className: string) {
...
function addAreaPath(className: string, d: string) {
  if (!d) return;
  addPath(d, className);
}

function addPoints(data: Point[], scale: Scale) {
...
function addLine(x1: number, y1: number, x2: number, y2: number, className: string) {
...
function addRect(x: number, y: number, width: number, height: number, className: string) {
...
function addText(x: number, y: number, value: string, className: string, anchor: "start" | "middle" | "end" = "start") {
